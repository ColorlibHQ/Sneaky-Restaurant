...
class sneakyrestaurant_author_widget extends WP_Widget {
...
'sneakyrestaurant_author_widget',


// Widget name will appear in UI
esc_html__( '[ sneakyrestaurant ] Author Widget', 'sneakyrestaurant' ),

// Widget description
array( 'description' => esc_html__( 'Add author content', 'sneakyrestaurant' ), )
);
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Author Name', 'sneakyrestaurant' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'designation' ) ); ?>"><?php esc_html_e( 'Designation:' ,'sneakyrestaurant'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'textarea' ) ); ?>"><?php esc_html_e( 'Short Descriptions:' ,'sneakyrestaurant'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Facebook Url' ) ); ?>"><?php esc_html_e( 'Facebook Url:' ,'sneakyrestaurant'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Twitter Url' ) ); ?>"><?php esc_html_e( 'Twitter Url:' ,'sneakyrestaurant'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Dribbble Url' ) ); ?>"><?php esc_html_e( 'Dribbble Url:' ,'sneakyrestaurant'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Behance Url' ) ); ?>"><?php esc_html_e( 'Behance Url:' ,'sneakyrestaurant'); ?></label>
...
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>"><?php _e( 'Image', 'sneakyrestaurant' ); ?>:</label>
	<div class="sneakyrestaurant-media-container">
		<div class="sneakyrestaurant-media-inner">
			...
			<span class="sneakyrestaurant-no-image" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-noimg" <?php echo wp_kses_post( $no_img_style ); ?>><?php _e( 'No image selected', 'sneakyrestaurant' ); ?></span>
		</div>
	
	<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" value="<?php echo esc_attr( $image ); ?>" class="sneakyrestaurant-media-url" />

	<input type="button" value="<?php echo _e( 'Remove', 'sneakyrestaurant' ); ?>" class="button sneakyrestaurant-media-remove" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-remove" <?php echo wp_kses_post( $img_style ); ?> />

	<?php $button_text = ( $image != '' ) ? __( 'Change Image', 'sneakyrestaurant' ) : __( 'Select Image', 'sneakyrestaurant' ); ?>
	<input type="button" value="<?php echo esc_html( $button_text ); ?>" class="button sneakyrestaurant-media-upload" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-button" />
	<br class="clear">
	</div>
</p>
<style>
.sneakyrestaurant-media-container {
	width: 98%;
}

.sneakyrestaurant-media-inner {
...
.sneakyrestaurant-media-inner img {
	max-width: 100%;
	height: auto;
}

.sneakyrestaurant-media-url {
	display: none;
}

.sneakyrestaurant-media-remove {
	float: left;
	width: 48%;
}

.sneakyrestaurant-media-upload {
	float: right;
	width: 48%;
}
</style>
...
jQuery(function($){
    ...
    wpshed_image_upload( '.sneakyrestaurant-media-upload' );
    ...
    wpshed_image_remove( '.sneakyrestaurant-media-remove' );
...
} // Class sneakyrestaurant_author_widget ends here


// Register and load the widget
function sneakyrestaurant_author_load_widget() {
	register_widget( 'sneakyrestaurant_author_widget' );
}
add_action( 'widgets_init', 'sneakyrestaurant_author_load_widget' );